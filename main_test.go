...
func TestSVD(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			U, S, V := SVD(tt.matrix)
			// check lengths
			if len(U) != len(tt.wantU) || len(S) != len(tt.wantS) || len(V) != len(tt.wantV) {
				t.Errorf("SVD() dimensions mismatch: got U %v, S %v, V %v; want U %v, S %v, V %v", U, S, V, tt.wantU, tt.wantS, tt.wantV)
				return
			}
			// Compare U
			for i := range U {
				for j := range U[i] {
					if math.Abs(U[i][j]-tt.wantU[i][j]) > 1e-3 {
						t.Errorf("SVD() U = %v, want %v", U, tt.wantU)
						return
					}
				}
			}
			...
			for i := range S {
				for j := range S[i] {
					if math.Abs(S[i][j]-tt.wantS[i][j]) > 1e-3 {
						t.Errorf("SVD() S = %v, want %v", S, tt.wantS)
						return
					}
				}
			}
			...
			for i := range V {
				for j := range V[i] {
					if math.Abs(V[i][j]-tt.wantV[i][j]) > 1e-3 {
						t.Errorf("SVD() V = %v, want %v", V, tt.wantV)
						return
					}
				}
			}
